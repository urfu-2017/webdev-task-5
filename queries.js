...
module.exports = class Queries {
    ...
    getSouvenrisCount({ country, rating, price }) {
        ...
        return this._Souvenir.find({ $and: [{ country: country },
            { rating: { $gte: rating } },
            { price: { $lte: price } }] })
            .count();
    ...
    searchSouvenirs(substring) {
        ...
        return this._Souvenir.find({ name: { $regex: new RegExp(substring), $options: 'i' } });
    }
    ...
    ratingRecount(souvenir) {
        let sumRating = 0;
        let countReviews = souvenir.reviews.length;
        for (let review of souvenir.reviews) {
            sumRating += review.rating;
        }

        return sumRating / countReviews;
    }
    ...
    async addReview(souvenirId, { login, rating, text }) {
        ...
        souvenir.rating = Math.round(this.ratingRecount(souvenir), 1);
    ...
    async getCartSum(login) {
        ...
        for (let souvenir of cart.items) {
            souvenirsData[souvenir.souvenirId] = souvenir.amount;
        }
        let souvenirs = await this._Souvenir.find({ _id: { $in: Object.keys(souvenirsData) } });
        for (let souvenir of souvenirs) {
            totalPrice += souvenir.price * souvenirsData[souvenir._id];
        }
